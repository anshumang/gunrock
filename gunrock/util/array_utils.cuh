...
public:
    ...
    cudaError_t Allocate(SizeT size, unsigned int target = HOST)
    {
        ...
        /*printf("Allocating %s on %d ",name.c_str(),target);
        printf(((target & HOST) == HOST)?"HOST":"");
        printf(((target & DEVICE) == DEVICE)?"DEVICE":"");
        printf(", size = %d\n",  size);fflush(stdout);*/
    ...
    {
        //printf("%s getting pointer on %d\n",name.c_str(),target);fflush(stdout);
        if (target == HOST  ) 
        {
